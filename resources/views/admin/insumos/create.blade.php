...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.insumo.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.insumos.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="nombre_insumo">{{ trans('cruds.insumo.fields.nombre_insumo') }}</label>
                <input class="form-control {{ $errors->has('nombre_insumo') ? 'is-invalid' : '' }}" type="text" name="nombre_insumo" id="nombre_insumo" value="{{ old('nombre_insumo', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('unidad_insumo') ? 'is-invalid' : '' }}" type="number" name="unidad_insumo" id="unidad_insumo" value="{{ old('unidad_insumo', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lugar_insumo') ? 'is-invalid' : '' }}" name="lugar_insumo_id" id="lugar_insumo_id" required>
                    @foreach($lugar_insumos as $id => $entry)
                        <option value="{{ $id }}" {{ old('lugar_insumo_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
