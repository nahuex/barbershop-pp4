    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.herramienta.update", [$herramientum->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nombre_herramienta') ? 'is-invalid' : '' }}" type="text" name="nombre_herramienta" id="nombre_herramienta" value="{{ old('nombre_herramienta', $herramientum->nombre_herramienta) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('unidad_herramienta') ? 'is-invalid' : '' }}" type="number" name="unidad_herramienta" id="unidad_herramienta" value="{{ old('unidad_herramienta', $herramientum->unidad_herramienta) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lugar_herramienta') ? 'is-invalid' : '' }}" name="lugar_herramienta_id" id="lugar_herramienta_id" required>
                    @foreach($lugar_herramientas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lugar_herramienta_id') ? old('lugar_herramienta_id') : $herramientum->lugar_herramienta->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
