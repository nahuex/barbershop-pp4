    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.turnos.update", [$turno->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cliente_turno') ? 'is-invalid' : '' }}" name="cliente_turno_id" id="cliente_turno_id" required>
                    @foreach($cliente_turnos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('cliente_turno_id') ? old('cliente_turno_id') : $turno->cliente_turno->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('barbershop_turno') ? 'is-invalid' : '' }}" name="barbershop_turno_id" id="barbershop_turno_id" required>
                    @foreach($barbershop_turnos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('barbershop_turno_id') ? old('barbershop_turno_id') : $turno->barbershop_turno->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('barbero_turno') ? 'is-invalid' : '' }}" name="barbero_turno_id" id="barbero_turno_id" required>
                    @foreach($barbero_turnos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('barbero_turno_id') ? old('barbero_turno_id') : $turno->barbero_turno->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('servicio_turno') ? 'is-invalid' : '' }}" name="servicio_turno_id" id="servicio_turno_id" required>
                    @foreach($servicio_turnos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('servicio_turno_id') ? old('servicio_turno_id') : $turno->servicio_turno->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('fecha_turno') ? 'is-invalid' : '' }}" type="text" name="fecha_turno" id="fecha_turno" value="{{ old('fecha_turno', $turno->fecha_turno) }}" required>
