...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.barbero.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.barberos.update", [$barbero->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nombre_barbero">{{ trans('cruds.barbero.fields.nombre_barbero') }}</label>
                <input class="form-control {{ $errors->has('nombre_barbero') ? 'is-invalid' : '' }}" type="text" name="nombre_barbero" id="nombre_barbero" value="{{ old('nombre_barbero', $barbero->nombre_barbero) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('correo_barbero') ? 'is-invalid' : '' }}" type="email" name="correo_barbero" id="correo_barbero" value="{{ old('correo_barbero', $barbero->correo_barbero) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('telefono_barbero') ? 'is-invalid' : '' }}" type="number" name="telefono_barbero" id="telefono_barbero" value="{{ old('telefono_barbero', $barbero->telefono_barbero) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('servicios_barberos') ? 'is-invalid' : '' }}" name="servicios_barberos[]" id="servicios_barberos" multiple>
                    @foreach($servicios_barberos as $id => $servicios_barbero)
                        <option value="{{ $id }}" {{ (in_array($id, old('servicios_barberos', [])) || $barbero->servicios_barberos->contains($id)) ? 'selected' : '' }}>{{ $servicios_barbero }}</option>
                    @endforeach
                </select>
